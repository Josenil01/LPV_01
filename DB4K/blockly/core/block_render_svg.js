...
Blockly.BlockSvg.prototype.renderDraw_ = function (iconWidth, inputRows) {
  ...
  this.renderDrawTop_(steps, highlightSteps, connectionsXY,
    inputRows.rightEdge);
  var cursorX = this.renderDrawRight_(steps, highlightSteps, inlineSteps,
    highlightInlineSteps, connectionsXY, inputRows, iconWidth);
  this.renderDrawBottom_(steps, highlightSteps, connectionsXY, cursorX,inputRows);
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function (steps, highlightSteps,
  ...
  for (var y = 0, row; row = inputRows[y]; y++) {
    ...
    if (this.isCollapsed()) {
      ...
      this.renderFields_(input.fieldRow, fieldX, fieldY);
      // addicionado no value input o encaixe femea do lado direito
      steps.push('v', 15);
      steps.push(Blockly.BlockSvg.TAB_PATH_DOWN);
      //var v = row.height - Blockly.BlockSvg.TAB_HEIGHT;//original
      steps.push('v', 15);
      if (this.RTL) {
      ...
      this.renderFields_(input.fieldRow, fieldX, fieldY);
      // dummy input alterado lado direito recebe conector femea
      // steps.push('v', row.height);//original
      steps.push('v ', 15);
...
Blockly.BlockSvg.prototype.renderDrawBottom_ =
  function (steps, highlightSteps, connectionsXY, cursorX, inputRows) {
    ...
    if (this.squareBottomLeftCorner_) {
      steps.push('H 0');
      if (!this.RTL) {
        highlightSteps.push('M', '0.5,' + (this.height - 0.5));
      }
    } else {
      ...
      if (!this.RTL) {
        highlightSteps.push('M', Blockly.BlockSvg.DISTANCE_45_INSIDE + ',' +
          (this.height- Blockly.BlockSvg.DISTANCE_45_INSIDE));
        highlightSteps.push('A', (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ',' +
          (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ' 0 0,1 ' +
          '0.5,' + (this.height - Blockly.BlockSvg.CORNER_RADIUS));
      }
    }
  };
...
Blockly.BlockSvg.prototype.renderDrawLeft_ =
  function (steps, highlightSteps, connectionsXY) {
    //adicionado para criar orelhinha do lado direito.
    if (this.previousConnection)
    {
      steps.push('V', Blockly.BlockSvg.TAB_HEIGHT + 15);
